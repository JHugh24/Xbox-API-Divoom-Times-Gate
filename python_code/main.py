...
# Details for requesting authorization code, the ampersand (&) is used as a parameter separator when constructing the authorization URL, the question mark (?) indicates that what follows is the query string
...
redirect_response = input("Paste the redirect URL here: ") # Requests user to paste redirect response URL that has been given from Microsoft containing the authorization code. Change: Work to automate this portion so no input is needed, probably using requests.get?
...
auth_code = parse_qs(parsed_url.query)['code'][0] # parse_qs parses the query strings parameters into a dictionary of "parameter: [value]" pairs, specifically for this function "code: [0]" extracts the authorization code from the query string 
...
token_data = { # token_data is a dictionary containing the data being sent in the request for the token
 ...
 #   'client_secret': CLIENT_SECRET, # Client Secret is essentially the password created within Azure that authenticates the app to be used in the request
    'code': auth_code, # The authorization code that was obtained from the redirect response url after a successful authorization through Microsofts OAuth
...
if xbox_friends_response.status_code == 200:
    ...
    online_friends = [
        friend for friend in friends_list['people']
        if friend['presenceState'] == 'Online'
    ]

    print('Online Friends:')
    for friend in online_friends:
        print(f"Gamertag: {friend['gamertag']}, Status: {friend['presenceText']}")
    else:
        print("Failed to retrieve friends list:", xbox_friends_response.status_code, xbox_friends_response.text)
